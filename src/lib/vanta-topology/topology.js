...
//
// Forked locally from vanta@0.5.24 (MIT) to cut its per-frame cost. The
// upstream sketch draws 4,500 particles on a 2D canvas every frame,
// uncapped, forever — measured as the actual cause of lag on this site's
// Hero section. The only two changes from upstream are `number_of_particles`
// (4500 -> 450) and `flow_cell_size` (10 -> 20, quarters the one-time flow
// grid setup cost). Everything else is untouched so future diffs against
// upstream stay small.

import P5Base, {VANTA} from './p5Base.js'
...
class Effect extends P5Base {
  ...
  onInit() {
    ...
    let sketch = function(p) {
      ...
      let number_of_particles = 450 // was 4500
      let particles = []
