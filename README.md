...
Plain backgrounds, vibrant accents, light and dark. `Nav.tsx` is a static
strip, not a fixed header: just the name and the theme toggle
(`ThemeToggle.tsx`), no section links. The site defaults to the OS
preference via `prefers-color-scheme` the first time a visitor shows up,
then remembers whatever they pick in `localStorage`. An inline script in
`layout.tsx` sets `data-theme` on `<html>` before first paint so there's no
flash.
...
src/
  ...
  components/
    ...
    Nav.tsx           static strip: name + theme toggle, not fixed
